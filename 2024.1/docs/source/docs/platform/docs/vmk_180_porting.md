...
12. In the Project Manager pane on the left-hand side under Program and Debug, click on **Generate Device Image**. The device image (pdi) for the design will be generated and available at `$working_dir/vivado/<platform_name>/project/impl_1/mrmac_subsys_wrapper.pdi`.
...
4. Run the below commands to build and package the wic image in compressed format:
   ...
   /images/linux   
   ```
5. Copy the images (`BOOT.BIN , rootfs.cpio.gz.u-boot , Image and boot.scr` ) to the FAT32 formatted SD card and insert the card in SD card slot to run the design. This image is functionally equivalent to the prebuilt sdcard image provided with package.
